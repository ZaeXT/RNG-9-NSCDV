...
std::mutex mtx;
//QString name[] = {"巴雷!", "包宇恒", "陈晶晶", "陈佩琪", "黄彦玮", "姜王迪", "李精锐", "刘静雯", "刘周心", "柳子恒",
//                  "马佳豪", "马文军", "潘梓豪", "沈一!", "殳楷博", "王羽郴", "王哲宇", "吴国祥", "吴家兴", "肖曦文",
//                  "徐景飏", "徐秋雨", "杨坤元", "姚杭晨", "俞佳扬", "张弛斌", "张创!", "张帅!", "张天浩", "张鑫琳",
//                  "张伊靓", "张奕阳", "张宇喆", "钟韩易", "朱妍!", "朱钰欣", "朱正天", "姚董轩"};
...
int main(int argc, char *argv[]) {
    ...
    QObject::connect(&ter, &QTimer::timeout, [&]() {
        if (ScrollFlag && displayChar == 0 && mode == 0) {
            text_show->setText(name[upch][0] + spacekey + rndChar[gen_rnd(0, int(rndChar.size()) - 1)] + spacekey +
                               rndChar[gen_rnd(0, int(rndChar.size()) - 1)]);
        } else if (ScrollFlag && displayChar == 1 && mode == 0) {
            text_show->setText(
                    name[upch][0] + spacekey + name[upch][1] + spacekey + rndChar[gen_rnd(0, int(rndChar.size()) - 1)]);
    ...
    QObject::connect(buttom1600, &QPushButton::clicked, [&] {
        ...
        if (Enreply == QMessageBox::Yes) {
            ...
            QTimer dWidget;
            dWidget.singleShot(100, [&]() { enhanced->deleteLater(); });
        ...
        mer.singleShot(i * 750, [=]() mutable {
            ...
            QObject::connect(&ter[i], &QTimer::timeout, [=]() mutable {
                ...
                if (ScrollFlag[i] && displayChar[i] == 0 && mode[i] == 0) {
                    text_show[i]->setText(
                            name[upch][0] + spacekey + rndChar[gen_rnd(0, int(rndChar.size()) - 1)] + spacekey +
                            rndChar[gen_rnd(0, int(rndChar.size()) - 1)]);
                    ...
                    text_show[i]->setText(name[upch][0] + spacekey + name[upch][1] + spacekey +
                                          rndChar[gen_rnd(0, int(rndChar.size()) - 1)]);
                    // QTimer::singleShot(IntervalScroll_ier, [=]() mutable
                    //                    {
                    displayChar[i]++;
                    ...
                    text_show[i]->setText(
                            name[upch][0] + spacekey + name[gen_rnd()][1] + spacekey + name[gen_rnd()][2]);
                    // QTimer::singleShot(IntervalScroll_ier, [=]() mutable
                    //                    {
                    displayChar[i]++;
                    //  });
